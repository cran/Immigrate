...
LFE<-function(train_xx,train_yy,T=5){
  N<-nrow(train_xx)
  p<-ncol(train_xx)
  if ( (T<1)|(T>N)){
    stop("use wrong T")
  ...
  weight<-sapply(c(1:T),function(i){
    ...
    dis<-colSums((t(train_xx)-train_xx[k,])^2)
    dis_h<-tmp_yy*dis
    dis_m<-(1-tmp_yy)*dis
    nh<-which(dis_h==min(dis_h[dis_h>0]))[1]
    nm<-which(dis_m==min(dis_m[dis_m>0]))[1]
    MM<-abs(t(train_xx)-as.numeric(train_xx[nm,]))%*%
      (t(abs(t(train_xx)-as.numeric(train_xx[nm,]))))-
      abs(t(train_xx)-as.numeric(train_xx[nh,]))%*%
      (t(abs(t(train_xx)-as.numeric(train_xx[nh,]))))
    M<<-M+MM
  })
  eggvalue<-eigen(-M)
